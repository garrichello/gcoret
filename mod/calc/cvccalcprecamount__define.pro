...
;
;  PURPOSE:
;    Sample class. Implements calculation of a spatial field of total precipitation for time series of data
;
;  INHERITS:
;    cvcCalc
;
;  CALLING SEQUENCE:
;  1) create instance: 
;     oCalcTimeMean = obj_new('cvcCalcPrecAmount', sIniData)
;       where sIniData is a filled structure of type 'structIniData'
;  2) call function 'Run':
;     result = oCalcPrecAmount->Run()
...
;--------------------------------------------------------------------
FUNCTION cvcCalcPrecAmount::Init, in_sInputs, in_sOutputs, in_hLogFile

    return, self->cvcCalc::Init(in_sInputs, in_sOutputs, in_hLogFile)
END
;--------------------------------------------------------------------
PRO cvcCalcPrecAmount::Cleanup

    self->cvcCalc::Cleanup

END
;--------------------------------------------------------------------
FUNCTION cvcCalcPrecAmount::Run

    self->printLog, "(cvcCalcPrecAmount::Run) Started..."
    ...
    self->printLog, "(cvcCalcPrecAmount::Run) Get data..."
    ...
    for segIdx = 0, numTimeSeg - 1 do begin
      ...
      if (sResponse.gridType ne 'station') then begin ; for non-station data
        ...
        for i = 1, n_elements(sResponse.aTimes)-1 do begin
         ...
         ; Precipitation amount for each segment
        totCnt[*, *, segIdx] = cnt
        totSum [*, *, segIdx]= sum
   ...
   if (calcMode eq 'single') then begin
      ...
      if (calcMode eq 'multi') then begin
        ...
        self->printLog, '(cvcCalcPrecAmount) Error! Unknown calculation mode: ', mode
        return, -1
      endelse
    endelse
...
END
;--------------------------------------------------------------------
PRO cvcCalcPrecAmount__define

    struct = { cvcCalcPrecAmount, $
               INHERITS cvcCalc $ 
             }
END
