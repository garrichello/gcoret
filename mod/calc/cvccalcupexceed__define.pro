...
;  CALLING SEQUENCE:
;  1) create instance: 
;     oCalclowUpExceed = obj_new('cvcCalcupExceed', sIniData)
;       where sIniData is a filled structure of type 'structIniData'
;  2) call function 'Run':
;     result = oCalcUpExceed->Run()
;
...
;--------------------------------------------------------------------
FUNCTION cvcCalcUpExceed::Init, in_sInputs, in_sOutputs, in_hLogFile
  
    return, self->cvcCalc::Init(in_sInputs, in_sOutputs, in_hLogFile)
END
;--------------------------------------------------------------------
PRO cvcCalcUpExceed::Cleanup

    self->cvcCalc::Cleanup

END
;--------------------------------------------------------------------
PRO cvcCalcUpExceed::BPtoIPinterp, i, sResponse_IP, sResponse_BP, aIPreGrid, aBPreGrid, reLons, reLats
...
END
;--------------------------------------------------------------------
PRO cvcCalcUpExceed::Exseedance, aIPreGrid, aBPreGrid, missingVal, calcMode, valPos, tmpCnt, curData, curCnt  
   posIP = long(aIPreGrid ne missingVal)
   posBP = long(aBPreGrid ne missingVal)
   idxs = where(aIPreGrid * posIP * posBP gt aBPreGrid * posIP * posBP, complement = zdxs)
   case calcMode of
     'frequency': begin                   
     ...
     'intensity': begin
                    if (idxs[0] ne -1) then begin
                      curData[idxs] = curData[idxs] + (aIPreGrid[idxs] - aBPreGrid[idxs])
...
END
;--------------------------------------------------------------------
FUNCTION cvcCalcUpExceed::Run
    
    self->printLog, "(cvcCalcUpExceed::Run) Started..."
    ...
    self->printLog, "(cvcCalcUpExceed::Run) Get data..."
...
END
;--------------------------------------------------------------------
PRO cvcCalcUpExceed__define

    struct = { cvcCalcUpExceed, $
               INHERITS cvcCalc $ 
             }
END