...
;  CALLING SEQUENCE:
;  1) create instance: 
;     oCalcGSL = obj_new('cvcCalcSDDT', sIniData)
...
;--------------------------------------------------------------------
FUNCTION cvcCalcSDDT::Init, in_sInputs, in_sOutputs, in_hLogFile

    return, self->cvcCalc::Init(in_sInputs, in_sOutputs, in_hLogFile)
END
;--------------------------------------------------------------------
PRO cvcCalcSDDT::Cleanup

    self->cvcCalc::Cleanup

END
;--------------------------------------------------------------------
FUNCTION cvcCalcSDDT::Run
    
    self->printLog, "(cvcCalcSDDT::Run) Started..."
    ...
    self->printLog, "(cvcCalcSDDT::Run) Get data..."
    ...
    for segIdx = 0, numTimeSeg - 1 do begin
      ...
      if (sResponse.gridType ne 'station') then begin ; for non-station data
        if (segIdx eq 0) then begin
          ...
          totSDDT = fltarr(sz[1], sz[2], numTimeSeg)
        ...
        sddtArr = fltarr(size(sResponse.aData[*,*,0], /dim))
        curCnt = cnt
        for i = 1, n_elements(sResponse.aTimes)-1 do begin
          if (long(sResponse.aTimes[i] - 0.5) eq long(curDay)) then begin
            ...
            if (long(curDay) lt long(halfYear)) then begin   ; first period: from 1 January to 1 July
              ...
              sddtArr = sddtArr + (sumArr * cur1Pos * curPos - 5 * cur1Pos * curPos)
      ...
      totSDDT[*, *, segIdx] = sddtArr
    endif else begin ; for station data
      if (segIdx eq 0) then begin
        sz = size(sResponse.aData)
        totSDDT = fltarr(sz[1], numTimeSeg)
  ...
  if (calcMode eq 'single') then begin
      ...
      if (calcMode eq 'multi') then begin
        if (sResponse.gridType ne 'station') then begin ; for non-station data
          ...
          for segIdx = 0, numTimeSeg - 1 do begin
            sum = totSDDT[*, segIdx]
            idxs = where(totSDDT[*, segIdx] eq 0)
            if (idxs[0] ne -1) then sum[idxs] = sResponse.missingVal
            aDataArray[*, segIdx] = sum
          endfor
        endelse
      endif else begin
        self->printLog, '(cvcCalcSDDT) Error! Unknown calculation mode: ', mode
        return, -1
      endelse
    endelse
