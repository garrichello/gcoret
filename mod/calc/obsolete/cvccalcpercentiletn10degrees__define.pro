...
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTN10degrees::Init, in_sInputs, in_sOutputs
  
    return, self->cvcCalc::Init(in_sInputs, in_sOutputs)
END

;--------------------------------------------------------------------
PRO cvcCalcPercentileTN10degrees::Cleanup

    self->cvcCalc::Cleanup

END
;--------------------------------------------------------------------
PRO cvcCalcPercentileTN10degrees::BP10pVals, aBPData, aBPDate, curIPDay, valBPmissing, BP10pArray
      ...
      for i = 1, (size(aBPDate))[1]-1 do begin
        ...
        if (mn_BP eq mn_IP) and (dy_IP eq dy_BP) then begin
          ...
          for val = 1, pVal-1 do begin
            ...
            minIdx = where(aArray * aPos lt bArray * aPos)
...
END
;--------------------------------------------------------------------
PRO cvcCalcPercentileTN10degrees::BPtoIPgrid, BP10pArray, minIPArr, aBPlats, aBPlons, aIPlats, aIPlons, valBPmissing, aBPreGrid, aIPreGrid, lonArr, latArr 
...
END
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTN10degrees::Run
    ...
    for segIdx = 0, numTimeSeg_IP - 1 do begin
     ...
     ;       minIPCnt = long(minIPArr ne sResponse_IP.missingVal)
            ...
            valIdxs = where(aIPreGrid * minIPCnt lt aBPreGrid * minIPCnt)
...
END
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTN10degrees::GetResult
   return, self.sResult
END
;--------------------------------------------------------------------
...
pro cvcCalcPercentileTN10degrees__define
   struct = { cvcCalcPercentileTN10degrees, $
               INHERITS cvcCalc $
             }
end