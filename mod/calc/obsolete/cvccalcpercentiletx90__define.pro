...
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTX90::Init, in_sInputs, in_sOutputs
  
    return, self->cvcCalc::Init(in_sInputs, in_sOutputs)
END

;--------------------------------------------------------------------
PRO cvcCalcPercentileTX90::Cleanup

    self->cvcCalc::Cleanup

END
;--------------------------------------------------------------------
PRO cvcCalcPercentileTX90::BP10pVals, aBPData, aBPDate, curIPDay, valBPmissing, BP10pArray
    ...
    for segIdx = 0, (size(aBPDate))[2] - 1 do begin 
      ...
      for i = 1, (size(aBPDate))[1]-1 do begin
        ...
        if (mn_BP eq mn_IP) and (dy_IP eq dy_BP) then begin
          ...
          tmpBPArray[*,*,s] = maxBPArr
      ;    print, s, max(maxbparr)
...
END
;--------------------------------------------------------------------
PRO cvcCalcPercentileTX90::BPtoIPgrid, BP10pArray, minIPArr, aBPlats, aBPlons, aIPlats, aIPlons, valBPmissing, aBPreGrid, aIPreGrid, lonArr, latArr 
...
END
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTX90::Run
...
END
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTX90::GetResult
   return, self.sResult
END
;--------------------------------------------------------------------
...
pro cvcCalcPercentileTX90__define
   struct = { cvcCalcPercentileTX90, $
               INHERITS cvcCalc $
             }
end