...
;  CALLING SEQUENCE:
;  1) create instance: 
;     oCalcPercentileTX90 = obj_new('cvcCalcPercentileTX90', sIniData)
;       where sIniData is a filled structure of type 'structIniData'
;  2) call function 'Run':
;     result = oCalcPercentileTX90->Run()
;
...
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTX90degrees::Init, in_sInputs, in_sOutputs
  
    return, self->cvcCalc::Init(in_sInputs, in_sOutputs)
END

;--------------------------------------------------------------------
PRO cvcCalcPercentileTX90degrees::Cleanup

    self->cvcCalc::Cleanup

END
;--------------------------------------------------------------------
PRO cvcCalcPercentileTX90degrees::BP10pVals, aBPData, aBPDate, curIPDay, valBPmissing, BP10pArray
    ...
    for segIdx = 0, (size(aBPDate))[2] - 1 do begin 
      ...
      for i = 1, (size(aBPDate))[1]-1 do begin
        ...
        if (mn_BP eq mn_IP) and (dy_IP eq dy_BP) then begin
          ...
          tmpBPArray[*,*,0] = maxBPArr
          for val = 0, pVal-2 do begin
    ...
    pos2 = long(tmpBPArray[*,*,0] ne 1e20)
    pos1 = long(tmpBPArray[*,*,1] ne 1e20)
    BP10pArray = (0.9 * (tmpBPArray[*,*,1]) + 0.1 * (tmpBPArray[*,*,0])) * pos1 * pos2
...
END
;--------------------------------------------------------------------
PRO cvcCalcPercentileTX90degrees::BPtoIPgrid, BP10pArray, minIPArr, aBPlats, aBPlons, aIPlats, aIPlons, valBPmissing, aBPreGrid, aIPreGrid, lonArr, latArr 
...
END
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTX90degrees::Run
      ...
      if (sResponse_IP.gridType ne 'station') then begin ; for non-station data
        ...
        for i = 1, n_elements(sResponse_IP.aTimes)-1 do begin
          if (long(sResponse_IP.aTimes[i] - 0.5) eq long(curIPDay)) then begin
            ...
            valIdxs = where(aIPreGrid * maxIPCnt gt aBPreGrid * maxIPCnt)
...
END
;--------------------------------------------------------------------
FUNCTION cvcCalcPercentileTX90degrees::GetResult
   return, self.sResult
END
;--------------------------------------------------------------------
...
pro cvcCalcPercentileTX90degrees__define
   struct = { cvcCalcPercentileTX90degrees, $
               INHERITS cvcCalc $
             }
end