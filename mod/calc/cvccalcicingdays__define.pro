...
;
;  DESCRIPTION:
;    Annual count of days when daily maximum temperature less than 0 C
;
;  PURPOSE:
;    Sample class. Implements calculation of a spatial field of icing days for time series of data
;    (Annual count of days when daily maximum temperature less than 0 C)
;
;  INHERITS:
;    cvcCalc
...
;  CALLING SEQUENCE:
;  1) create instance: 
;     oCalcIcingDays = obj_new('cvcCalcIcingDays', sIniData)
;       where sIniData is a filled structure of type 'structIniData'
;  2) call function 'Run':
;     result = oCalcIcingDays->Run()
;
;--------------------------------------------------------------------
FUNCTION cvcCalcIcingDays::Init, in_sInputs, in_sOutputs, in_hLogFile
  
    return, self->cvcCalc::Init(in_sInputs, in_sOutputs, in_hLogFile)
END

;--------------------------------------------------------------------
PRO cvcCalcIcingDays::Cleanup

    self->cvcCalc::Cleanup

END
FUNCTION cvcCalcIcingDays::Run

self->printLog, "(cvcCalcIcingDays::Run) Started..."
    ...
    self->printLog, "(cvcCalcIcingDays::Run) Get data..."
...
if (calcMode eq 'single') then begin
      ...
      if (calcMode eq 'multi') then begin
        ...
        self->printLog, '(cvcCalcIcingDays) Error! Unknown calculation mode: ', mode
        return, -1
      endelse
    endelse
...
pro cvcCalcIcingDays__define
   struct = { cvcCalcIcingDays, $
               INHERITS cvcCalc $
             }
end
