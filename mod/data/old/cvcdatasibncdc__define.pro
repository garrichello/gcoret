...
;
;--------------------------------------------------------------------
FUNCTION cvcDataSIBNCDC::Init, in_sDatasetDesc, in_modulePath, in_oGlobal
...
;+
; :Description:
;    Reads data array from NCDC dataset restricted for Siberia region. Accepts time range.
;
; :Params:
...
; :Author: garry
;-
FUNCTION cvcDataSIBNCDC::Read, in_aArea, in_aTimeRng, in_level, out_sData

    missingVal = -999.0
    
    asCurFileInfo = *(*self.pDataFile).pInfo

    aLats = asCurFileInfo.attribute_4
    aLons = asCurFileInfo.attribute_5
...
;      self->printLog, i
      ...
      aAllData[i, pos] = saData[idxsTime].attribute_2
...
END
;--------------------------------------------------------------------
FUNCTION cvcDataSIBNCDC::Prepare, in_varName, in_sTimeSeg, in_aLevel, in_sRegion
...
END
;--------------------------------------------------------------------
PRO cvcDataSIBNCDC__define

    struct = { cvcDataSIBNCDC, $
               INHERITS cvcData $
             }
END
