...
;
;--------------------------------------------------------------------
FUNCTION cvcDataJRA25nc::Init, in_sDatasetDesc, in_modulePath, in_oGlobal
...
;+
; :Description:
;    Reads data array from JRA25-file. Accepts time range.
;
; :Params:
...
; :Author: garry
;-
FUNCTION cvcDataJRA25nc::Read, in_aArea, in_aTimeRng, in_level, out_sData
...
;          self->printLog, "(cvcDataJRA25::Read) Reading file: ", (*self.pDataFile)[levIdx, fileIdx].name  
;          self->printLog, "(cvcDataJRA25::Read) Requesting variable: ", self.sRequest.varName, ", level: ", in_level, ", time: ", in_aTimeRng
;        endif
;      
;        recId = where(strcmp(asCurFileInfo.info.quantity, self.sRequest.varName) and $
;                    (asCurFileInfo.info.date ge in_aTimeRng[0]) and $
;                    (asCurFileInfo.info.date le in_aTimeRng[1]) and $
;                    (strmid(asCurFileInfo.info.gridid, 4) eq self->Name2Level(in_level)))    
;      
...
;    ; fix missing values
;    missingVal = -999.0
;    idxs = where(aAllData eq 9.999E20)
...
END
;--------------------------------------------------------------------
FUNCTION cvcDataJRA25nc::Prepare, in_varName, in_sTimeSeg, in_aLevel, in_sRegion
...
;    unitsPos = strpos(in_sTimeSeg.step, 'h')
;    hoursStep = fix(strmid(in_sTimeSeg.step, 0, unitsPos))
;    self.numberOfDataFiles = (julday(mn2, dy2, yr2, hr2) - julday(mn1, dy1, yr1, hr1)) * 24 / hoursStep
;       
...
END
;--------------------------------------------------------------------
PRO cvcDataJRA25nc__define

    struct = { cvcDataJRA25nc, $
               INHERITS cvcData $
             }
END